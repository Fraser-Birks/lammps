/* ----------------------------------------------------------------------
   ...
   certain riioghts in this software.  This software is distributed under
...
PairUF3mix::PairUF3mix(LAMMPS *lmp) :
...
void PairUF3mix::settings(int narg, char **arg)
{
  ...
  if (nbody_flag == 2) {
    ...
    error->all(FLERR, "Pair style uf3/mix not (yet) implemented for {}-body terms", nbody_flag);
}

/* ----------------------------------------------------------------------
 *    set coeffs for one or more type pairs
 * ---------------------------------------------------------------------- */
void PairUF3mix::coeff(int narg, char **arg)
{
  if (narg != 4 + atom->ntypes)
    error->all(FLERR, "Invalid number of arguments uf3/mix in pair coeffs.");
...
{
  allocated = 1;
  // const int num_of_types = atom->ntypes;
  const int num_of_elements = atom->ntypes;
...
void PairUF3mix::uf3_read_unified_pot_file(char *potf_name)
{
...
void PairUF3mix::init_list(int /*id*/, class NeighList *ptr)
{
  list = ptr;
}
...
void PairUF3mix::create_bsplines()
{
  ...
  if (spacing_type) {
    get_starting_index_2b = &PairUF3mix::get_starting_index_nonuniform_2b;
    if (pot_3b) get_starting_index_3b = &PairUF3mix::get_starting_index_nonuniform_3b;
  } else {
    get_starting_index_2b = &PairUF3mix::get_starting_index_uniform_2b;
    if (pot_3b) get_starting_index_3b = &PairUF3mix::get_starting_index_uniform_3b;
  }
...
int PairUF3mix::get_starting_index_uniform_2b(int i, int j, double r)
{
  return 3 + (int) ((r - n2b_knots_array[i][j][0]) / (knot_spacing_2b[i][j]));
}

int PairUF3mix::get_starting_index_uniform_3b(int i, int j, int k, double r, int knot_dim)
{
  return 3 +
...
int PairUF3mix::get_starting_index_nonuniform_2b(int i, int j, double r)
{
...
int PairUF3mix::get_starting_index_nonuniform_3b(int i, int j, int k, double r, int knot_dim)
{
...
void PairUF3mix::compute(int eflag, int vflag)
{
...
double PairUF3mix::single(int /*i*/, int /*j*/, int itype, int jtype, double rsq,
