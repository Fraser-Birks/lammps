...
PairTableMix::PairTableMix(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = nullptr;
...
void PairTableMix::compute(int eflag, int vflag)
{
...
void PairTableMix::settings(int narg, char **arg)
{
...
void PairTableMix::coeff(int narg, char **arg)
{
  ...
  null_table(tb);
  if (me == 0) read_table(tb, arg[2], arg[4]);
  bcast_table(tb);
  ...
  // set table cutoff
  if (narg == 6)
    tb->cut = utils::numeric(FLERR, arg[5], false, lmp);
  else if (tb->rflag)
    tb->cut = tb->rhi;
  else
...
double PairTableMix::init_one(int i, int j)
{
...
void PairTableMix::read_table(Table *tb, char *file, char *keyword)
{
...
void PairTableMix::bcast_table(Table *tb)
{
...
void PairTableMix::spline_table(Table *tb)
{
...
void PairTableMix::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairTableMix::compute_table(Table *tb)
{
  int tlm1 = tablength - 1;
...
void PairTableMix::null_table(Table *tb)
{
...
void PairTableMix::free_table(Table *tb)
{
...
void PairTableMix::spline(double *x, double *y, int n, double yp1, double ypn, double *y2)
{
  int i, k;
  double p, qn, sig, un;
  auto u = new double[n];
...
double PairTableMix::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo, khi, k;
  double h, b, a, y;
...
void PairTableMix::write_restart(FILE *fp)
{
...
void PairTableMix::read_restart(FILE *fp)
{
...
void PairTableMix::write_restart_settings(FILE *fp)
{
...
void PairTableMix::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairTableMix::single(int /*i*/, int /*j*/, int itype, int jtype, double rsq,
...
void *PairTableMix::extract(const char *str, int &dim)
{
